...
\author{Lukas Chavez\footnote{lchavez@liai.org}}
...
@
\Biocpkg{MEDIPS} will replace all reads which map to exactly the same start and end positions on the same strand by only one representative:  
<<echo=T, eval=T, results=hide>>=
uniq=TRUE
@
...
\item
\texttt{quantile}: DNA enrichment assays may have different efficencies. To normalize for this effect, quantile normalization can be enabled by setting this parameter to TRUE (default=FALSE). It has been proposed \cite{Seumois} that quantile normalization can correct for varying DNA enrichment efficiencies. When enabled, quantile normalization will be applied to the count table (including all genomic windows and all samples) prior to testing for differential coverage between conditions. 
\warning{The counts - but not rpkm and rms values - of the returned result table will be quantile normalized. If quantile normalization has been enabled, the results of the differential coverage analysis (fold change, p-values etc.) are based on quantile normalized count data.} 
\end{itemize} 
