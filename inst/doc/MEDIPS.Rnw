...
However, MEDIPS provides several functionalities for the analysis of any other kind of quantitative sequencing data (e.g. ChIP-seq, MBD-seq, CMS-seq and others) including
calculation of differential coverage between groups of samples as well as saturation and correlation analyses.
 
In detail, MEDIPS adresses the following aspects in the context of IP-seq data analysis: 

\begin{itemize}
\item 
estimating the reproducibility for obtaining full genome short read coverage profiles (saturation analysis), 
\item 
calculating genome wide sequencing signal densities at a user specified resolution (counts, rpkm),
\item 
calculating differentially coverage comparing two groups of IP-seq samples,
\item 
correcting for copy number variations putative present in the genomic background of the samples based on Input samples (if available),
\item 
...
\item 
calculating a CpG enrichment factor as a quality control for the MeDIP specific immunoprecipitation,
\item 
...
MEDIPS starts where the mapping tools stop (typically bam files) and can be used for any genome of interest, limited by the available genomes within the Bioconductor \texttt{BSgenome} package, only.
In case a genome of interest is not available as a \texttt{BSgenome} package but the sequence of the genome is available, a custom \texttt{BSgenome} package can be generated, 
please see the "How to forge a BSgenome data package" manual of the \texttt{BSgenome} package.
...
Hence, we apologize for any inconveniences, but we believe that these updates were necessary w.r.t. methodology, functionality, and performance (speed and memory usage).
...
but also allows to process any other kind of quantitative sequencing data (e.g. ChIP-seq) in order to calculate genome wide differential coverage. 
Differential coverage can then be interpreted as either differential methylation (MeDIP/MBD or ChIP for histone modifications), differential hydroxymethylation (CMS), 
...
<<echo=TRUE, eval=F, keep.source=FALSE>>=
MEDIPS.plotCalibrationPlot(CSet=CS, input=F, main="Calibration Plot", MSet=hESCs_MeDIP[[1]], plot_chr="chr22",  rpkm=T,  xrange=T)
@
...
\item 
\texttt{input}: specifies whether the given data set at \texttt{MSet} is MeDIP or Input data. In case of Input data, no linear regression will be calculated.
\item 
\texttt{main}: the main of the figure
\item 
\texttt{MSet}: the MEDIPS SET (or INPUT SET, respectively)
\item 
\texttt{plot\_chr}: the chromsome for which the calibration plot will be generated
\item 
\texttt{rpkm}: specifies, if the MeDIP data range will be plotted in count or rpkm format
\item 
