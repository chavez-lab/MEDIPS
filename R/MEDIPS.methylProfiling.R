...
#MEDIPS.methylProfiling=function ()
{

	.Deprecated("MEDIPS.meth") 
...
#	    
#	    matrix=suppressWarnings(.Call("roiprofile",input,as.numeric(select),as.matrix(ROI2),as.integer(chr_binposition), data1,data2 ,environment(wilcox.test),wilcox.test,environment(var),var,environment(math),math,t.test,environment(t.test),as.numeric(factor(chr_names(data1)))), PACKAGE=MEDIPS)
...
#matrix=suppressWarnings(.Call("profile",as.numeric(select),input,as.double(step),as.double(frame_size),data1,data2,environment(wilcox.test),wilcox.test,environment(var),var,environment(math),math,t.test,environment(t.test)), PACKAGE=MEDIPS)
#    }
#    cat("Postprocessing...\n")
