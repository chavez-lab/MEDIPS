...
##Param:	Set, (output)file, format, descr
##Output:	Wiggle file
##Modified:	11/10/2011
##Author:	Lukas Chavez

MEDIPS.exportWIG <-
function(Set=NULL, file=NULL, format="rpkm", descr=""){
	
	##Proof for correctness
	########################
	if(is.null(file)){stop("Must specify an output file!")}
	if(format != "count" & format!= "rpkm" & format != "pdensity"){stop("Format pareeter must be either count, rpkm or pdensity!")}
	
	##Set sample names and output data
	###################################
	if(format=="pdensity"){		
		if(class(Set)!="COUPLINGset"){stop("For exporting pattern densities, must specify a COUPLINGset object!")}
		...
		if(class(Set)!="MEDIPSset"){stop("For exporting count intensities, must specify a MEDIPSset object!")}
		window_size=window_size(Set)
		chr_names=chr_names(Set)
		chr_lengths=chr_lengths(Set)
		chromosomes=chr_names(Set)		
		...
		if(class(Set)!="MEDIPSset"){stop("For exporting rpm intensities, must specify a MEDIPSset object!")}
		window_size=window_size(Set)
		chr_names=chr_names(Set)
		chr_lengths=chr_lengths(Set)
		chromosomes=chr_names(Set)				
		output_data = (genome_count(Set)*10^9)/(window_size*number_regions(Set))
		header_out=paste("track type=wiggle_0 name=\"", descr, "\" description=\"", descr, "\" visibility=full autoScale=on color=0,0,255 maxHeightPixels=100:50:20 graphType=bar priority=20", sep="")	
	}
